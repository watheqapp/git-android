...
public class FragNavController {
    ...
    @NonNull
    private final List<ArrayDeque<Fragment>> mFragmentStacks;
    @NonNull
    ...
    private FragNavController(Builder builder, @Nullable Bundle savedInstanceState) {
        ...
        if (!restoreFromBundle(savedInstanceState, builder.mRootFragments)) {
            ...
            for (int i = 0; i < builder.mNumberOfTabs; i++) {
                ArrayDeque<Fragment> stack = new ArrayDeque<>();
     ...
     */
    public void switchTab(@TabIndex int index, @Nullable FragNavTransactionOptions transactionOptions) {
    ...
    public void pushFragment(@Nullable Fragment fragment) {
        if (fragment != null && mSelectedTabIndex != NO_TAB) {
            FragmentTransaction ft = createTransactionWithOptions();
     ...
     * @param fragment The fragment that is to be pushed
     */
    public void pushFragment(@Nullable Fragment fragment, boolean anim) {
        if(!anim)
     ...
     * @param transactionOptions Transaction options to be displayed
     */
    public void popFragment(@Nullable FragNavTransactionOptions transactionOptions) {
     ...
     * @param transactionOptions Transaction options to be displayed
     */
    public void popFragments(int popDepth, @Nullable FragNavTransactionOptions transactionOptions) {
        if (isRootFragment()) {
        ...
        if (fragment != null) {
            ...
            if (!mFragmentStacks.get(mSelectedTabIndex).isEmpty()) {
                fragment = commitIfStackNotEmpty(ft);
            } else {
                fragment = commitFragmentIfStackEmpty(ft);
                bShouldPush = true;
            }
        }
    ...
    private Fragment commitIfStackNotEmpty(FragmentTransaction ft) {
        Fragment fragment = mFragmentStacks.get(mSelectedTabIndex).peek();
        ft.add(mContainerId, fragment, fragment.getTag());
        ft.commit();
        return fragment;
    }

    private Fragment commitFragmentIfStackEmpty(FragmentTransaction ft) {
        Fragment fragment = getRootFragment(mSelectedTabIndex);
        ft.add(mContainerId, fragment, generateTag(fragment));
        ft.commit();
        return fragment;
    }
    ...
    public void clearStack(@Nullable FragNavTransactionOptions transactionOptions) {
        ...
        //Grab Current stack
        ArrayDeque<Fragment> fragmentStack = mFragmentStacks.get(mSelectedTabIndex);
        // Only need to start popping and reattach if the stack is greater than 1
        if (fragmentStack.size() > 1) {
            FragmentTransaction ft = createTransactionWithOptions(transactionOptions);
            Fragment fragment = reattachPreviousFragment(fragmentStack, ft);
    ...
    private Fragment reattachPreviousFragment(ArrayDeque<Fragment> fragmentStack, FragmentTransaction ft) {
        Fragment fragment;
    ...
    @CheckResult
    @Nullable
    public Deque<Fragment> getStack(@TabIndex int index) {
        if (index == NO_TAB) return new ArrayDeque<>();
        if (index >= mFragmentStacks.size()) {
            throw new IndexOutOfBoundsException("Can't get an index that's larger than we've setup");
        }
        return mFragmentStacks.get(index).clone();
    }
    ...
    @CheckResult
    @Nullable
    public Deque<Fragment> getCurrentStack() {
    ...
    public boolean isRootFragment() {
        Deque<Fragment> stack = getCurrentStack();
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        for (ArrayDeque<Fragment> stack : mFragmentStacks) {
    ...
    private boolean restoreFromBundle(@Nullable Bundle savedInstanceState, @Nullable List<Fragment> rootFragments) {
        ...
        try {
            stackArrays = new JSONArray(savedInstanceState.getString(EXTRA_FRAGMENT_STACK));
